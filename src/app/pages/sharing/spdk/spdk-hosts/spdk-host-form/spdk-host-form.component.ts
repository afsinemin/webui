...
@UntilDestroy()
@Component({
  selector: 'spdk-host-form',
  templateUrl: './spdk-host-form.component.html',
  styleUrls: ['./spdk-host-form.component.scss'],
...
export class SPDKHostFormComponent implements OnInit {
  private api = inject(ApiService);
  private formBuilder = inject(NonNullableFormBuilder);
  private errorHandler = inject(ErrorHandlerService);
  private formErrorHandler = inject(FormErrorHandlerService);
  slideInRef = inject<SlideInRef<SPDKHost | undefined, SPDKHost | null>>(SlideInRef);

  protected isLoading = signal(false);

  private existingHost = signal<SPDKHost | null>(null);

  protected isNew = computed(() => !this.existingHost());

  protected hashOptions$ = this.api.call('spdk.host.dhchap_hash_choices').pipe(singleArrayToOptions());
  protected dhGroupOptions$ = this.api.call('spdk.host.dhchap_dhgroup_choices').pipe(singleArrayToOptions());
  ...
  protected generateHostKey(): void {
    ...
    this.api.call('spdk.host.generate_key', [this.form.value.dhchap_hash, this.form.value.hostnqn])
      .pipe(
  ...
  protected generateTrueNasKey(): void {
    ...
    this.api.call('spdk.global.config').pipe(
      switchMap((config) => {
        return this.api.call('spdk.host.generate_key', [this.form.value.dhchap_hash, config.basenqn]);
      }),
    )
      .pipe(
  ...
  protected onSubmit(): void {
    ...
    const request$ = this.isNew()
      ? this.api.call('spdk.host.create', [payload])
      : this.api.call('spdk.host.update', [this.existingHost().id, payload]);
